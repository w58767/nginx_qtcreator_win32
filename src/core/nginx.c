...
main(int argc, char *const *argv)
{
    ...
    //解析输入参数,eg. nginx -s stop|start|restart
    ...
    /*初始化日志，并得到日志的文件句柄log(ngx_log_file.fd)*/
     ...
     * ngx_process_options()
     */
    /*init_cycle--Nginx的全局变量*/
    ngx_memzero(&init_cycle, sizeof(ngx_cycle_t));
    init_cycle.log = log;
    ngx_cycle = &init_cycle;
    /*内存池上创建一个默认大小1024的全局变量*/
     ...
     */
    /*初始化一个做循环冗余crc32校验的表，由此可以看出后续的循环冗余校验将采用高效的查表法*/
    ...
    /*初始化模块，对模块进行编号处理(存储所有模块的信息，包括自己开发的模块都会放到数组中)*/
    if (ngx_preinit_modules() != NGX_OK) {
        return 1;
    }

    /*初始化全局变量cycle(将会初始化很多的东东到全局变量cycle中，包括bind(), listen().是Nginx启动初始化的核心之处)*/
...
#endif
    /*如果是master进程，则创建pid文件。例如：/usr/local/nginx-1.4.7/nginx.pid*/
    if (ngx_create_pidfile(&ccf->pid, cycle->log) != NGX_OK) {
        return 1;
    }

    /*重定向错误日志的标准输出.之前输出到stderr,现在重定向到文件*/
    ...
    if (ngx_process == NGX_PROCESS_SINGLE) {
        ...
        /*如果是多进程(Master-Worker)模式,则进入master进程循环*/
