#define NGX_CONFIGURE " --with-cc=cl --builddir=objs --prefix= --conf-path=conf/nginx.conf --pid-path=logs/nginx.pid --http-log-path=logs/access.log --error-log-path=logs/error.log --sbin-path=nginx.exe --http-client-body-temp-path=temp/client_body_temp --http-proxy-temp-path=temp/proxy_temp --http-fastcgi-temp-path=temp/fastcgi_temp --with-cc-opt=-DFD_SETSIZE=1024 --with-pcre=lib/pcre --with-zlib=lib/zlib --with-select_module --with-ipv6 --without-poll_module"

#ifndef NGX_COMPILER
#define NGX_COMPILER  "cl 12.00.8168 for 80x86"
#endif
